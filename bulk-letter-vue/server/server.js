...
app.get('/api/recipients', requireLogin, async (_req, res, next) => {
  try { res.json((await readData()).recipients || []) } catch (error) { next(error) }
})
...
app.put('/api/recipients', requireLogin, async (req, res, next) => {
  ...
  try {
    ...
    const data = await readData()
    data.recipients = recipients.map((item) => ({ companyName: String(item.companyName || '').slice(0, 200), email: String(item.email || '').slice(0, 320) }))
    await writeData(data)
    res.json({ saved: data.recipients.length })
  } catch (error) { next(error) }
})
